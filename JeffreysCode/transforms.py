...
## 3D Transformations- analytic and symbolic
def rotx(th):
    """
    R = rotx(th)
    Parameters
...
def se3sym(R=sp.eye(3), p=sp.Matrix([0, 0, 0])):
    """
        T = se3(R, p)
        Description:
            Given a numpy 3x3 array for R, and a 1x3 or 3x1 array for p, 
...
def R2rpy(R):
    ...
    pitch = np.atan2(-R[2,0],sqrt((R[2,1])**2+(R[2,2])**2))
...
def R2quat(R):
    ...
    Returns:
    quaternion - 4 x 1 numpy matrix, quaternion representation of pose in the 
    format [nu, ex, ey, ez]
    """
    # TODO, see equation (2.34) and (2.35) on pg. 55, using functions like "sp.sqrt," and "sp.sign"

    return np.array([[0.5*sp.sqrt(R[0,0]+R[1,1]+R[2,2]+1)], 
                     [0.5*sp.sign(R[2,1]-R[1,2]+1e-10)*sp.sqrt(R[0,0]-R[1,1]-R[2,2]+1)],
                     [0.5*sp.sign(R[0,2]-R[2,1]+1e-10)*sp.sqrt(-R[0,0]+R[1,1]-R[2,2]+1)], 
                     [0.5*sp.sign(R[1,0]-R[0,1]+1e-10)*sp.sqrt(-R[0,0]-R[1,1]+R[2,2]+1)]])
