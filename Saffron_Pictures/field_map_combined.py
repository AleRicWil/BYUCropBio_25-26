...
# -------------------------------
# Combined folder
combined_folder = r'C:\Users\Alex R. Williams\Pictures\Saffron Data\10-27_11pm\Combined_Original'
csv_path = r'C:\Users\Alex R. Williams\Pictures\Saffron Data\10-27_11pm\captures.csv'
...
# Get available frame_ids directly from the Combined_Original folder filenames
# Each filename is {frame_id}_original.jpg, where frame_id is the RS1 timestamp
available_files = [f for f in os.listdir(combined_folder) if f.endswith('_original.jpg')]
available_frame_ids = [f.replace('_original.jpg', '') for f in available_files]

# Filter df to RS1 rows only (since frame_id corresponds to RS1 timestamp and GPS)
# and only those with matching combined images
df = df[(df['camera'] == 'RS1') & (df['timestamp'].isin(available_frame_ids))]

# Set frame_id to timestamp (since they match)
df['frame_id'] = df['timestamp']

# Compute raw grouped data (for raw mode)
raw_grouped = df.groupby(['latitude', 'longitude'])['frame_id'].agg(lambda x: sorted(list(set(x)))).reset_index()
...
if args.mode == 'filtered':
    ...
    # Group frame_ids by bins for popups
    grouped = df.groupby(['lat_bin', 'lon_bin'], observed=True)['frame_id'].agg(lambda x: sorted(list(set(x)))).reset_index()
    ...
    grouped = grouped[grouped['frame_id'].map(len) > 0]
...
class FieldViewer(tk.Tk):
    ...
    def on_hover(self, event):
        ...
        for lbin in lat_range:
            for obin in lon_range:
                ...
                frame_ids = row['frame_id'].iloc[0]  # Already sorted unique
                paths = [os.path.join(combined_folder, f"{fid}_original.jpg") for fid in frame_ids]
    ...
    def on_pick(self, event):
        ...
        if self.mode == 'raw' and artist == self.scatter:
            ind = event.ind[0]
            frame_ids = raw_grouped['frame_id'].iloc[ind]
            real_lat = raw_grouped['latitude'].iloc[ind]
            real_lon = raw_grouped['longitude'].iloc[ind]
            self.show_images(frame_ids, f"Lat {real_lat:.7f} | Lon {real_lon:.7f}")
        elif self.mode == 'filtered' and isinstance(artist, Rectangle):
            frame_ids = artist.frame_ids
            real_lat = artist.center_lat
            real_lon = artist.center_lon
            self.show_images(frame_ids, f"Cell Center Lat {real_lat:.7f} | Lon {real_lon:.7f}")
    ...
    def show_images(self, frame_ids, title):
        ...
        # Set up single column with weight
        scrollable_frame.grid_columnconfigure(0, weight=1)

        row = 0  # Start at top
        for frame_id in frame_ids:
            path = os.path.join(combined_folder, f"{frame_id}_original.jpg")
